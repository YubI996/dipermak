<div>
    ...
    <div class="col-md-10 mb-2">
        @if ($selectAll)
        <div>
            Anda telah mencentang  <strong>{{ $kegiatans->total() }}</strong> item.
        </div>
        @else
        <div>
            Anda telah mencentang <strong>{{ count($checked) }}</strong> item, Apa Anda ingin mencentang 
            <strong>{{ $kegiatans->total() }}</strong> item?
    ...
    <div class="table table-bordered table-striped table-hover">
        <table class="table table-bordered table-striped table-hover">
            ...
            @forelse($kegiatans as $kegiatan)
                <tr class="@if ($this->isChecked($kegiatan->id))
                    table-primary
                @endif">
                    <td><input type="checkbox" value="{{ $kegiatan->id }}" wire:model="checked"></td>                    
